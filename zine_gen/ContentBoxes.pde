...
class GenerativeBox extends ImageBox{
  
  String exe, command, imageLoc, compile;
  GenState currState = GenState.init;
  PApplet pApplet;
  Process currProcess;
  int width, height;
  String[] commands;
  ...
  public GenerativeBox(XML xml, Map<String, String> vars, String imageLoc, PApplet p){
    parseCommands(xml, vars);
    pApplet = p;
    this.imageLoc = imageLoc;
  }
  
  private void parseCommands(XML xml, Map<String, String> vars){
    XML[] xmlCommands = xml.getChildren("command");
    commands = new String[xmlCommands.length];
    for(int i = 0; i < xmlCommands.length; i++){
      commands[i] = parseAsText(xmlCommands[i], vars);
    }
  }
  ...
  public boolean isReady(){
    switch(currState){
      case init:
        try{
          println(commands[currCommand]);
          currProcess = Runtime.getRuntime().exec(commands[currCommand]);
        } catch(IOException ioe){
          errorMsg = ioe.getMessage();
          currState = GenState.error;
        }
        currState = GenState.running;
        break;
      case running:
        try{
          ...
          if (exitVal == 0){
            ...
            if (currCommand >= commands.length){
              currState = GenState.loadImage;
            } else {
              currState = GenState.init;
            }
          } else {
            errorMsg = "command " + currCommand + " exited with code " + exitVal;
            currState = GenState.error;
          }
  ...
  //void initCommands(){
  //  String dataDir = pApplet.dataPath("");
  ...
  //  sketchDir += "/";
  //  imageLoc = pApplet.dataPath("image"+((int)random(0,1000000))+".png");
  //  command = sketchDir + "build/"+exe+
  //" image="+imageLoc+
  //" width="+width+
  //" height="+height+
  ...
  //  compile =
  //processingJavaPath +
  //" --no-java --force --sketch="+parentDir+exe+
